...
import { checkUsageLimit, incrementUsage, createUsageLimitResponse } from './supabase/functions/_shared/usage.ts';
...
const validateInput = (input: ReelInput) => {
  ...
  if (!input.photoIds?.length && !input.videoIds?.length) missingFields.push('media (photos or videos)');
...
async function generateMediaOrder(
  ...
  // Prepare media descriptions with proper IDs
...
3. Use all provided media at least once
4. Return a valid JSON object with media elements and total duration
...
2. Each media element must be shown for at least 2 seconds
3. Use each media element at least once
4. Align visuals with relevant spoken content
    ...
    try {
      ...
      orderedMedia = {
        elements: parsedOutput.elements.map((element: any) => ({
          ...
          duration: Math.max(2, Number(element.duration)),
          source: element.source || mediaFiles.find(m => m.originalPath.includes(element.id))?.source || ''
        })),
